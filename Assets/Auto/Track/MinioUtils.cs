...
public static class MinioUtils
{
    ...
    public static string minioBucket = "track-dev";

    public static bool Upload(string filePath)
    {
        // S3 signature
        var minioPath = $"/{minioBucket}/{Path.GetFileName(filePath)}";
        var contentType = "application/octet-stream";
        var date = DateTime.UtcNow.ToString("R");
        var signature = $"PUT\n\n{contentType}\n{date}\n{minioPath}";
        var authorization = GetAuthorizationHeader(signature);
        return GTrackSDK.UploadFile(minioHost, minioBucket,  minioPath, filePath, authorization, date);
    }
